...
type ChartRuntimePhase = "idle" | "animating" | "done";
...
export function TriageConfidenceChart({
  ...
  const runtimeRef = useRef<{ signature: string | null; phase: ChartRuntimePhase }>({
    signature: null,
    phase: "idle"
  });
  const propsRef = useRef({
    workflowConfidence,
    confidenceFactors,
    tone: confidenceTone(workflowConfidence)
  });
  propsRef.current = {
    workflowConfidence,
    confidenceFactors,
  ...
  useEffect(() => {
    ...
    const { workflowConfidence: targetScore, confidenceFactors: factors, tone: chartTone } =
      propsRef.current;

    if (runtime.phase === "done" && runtime.signature === chartSignature) {
      setDisplayScore(Math.round(targetScore));
      setRevealed(true);
      return;
    }

    if (runtime.phase === "animating" && runtime.signature === chartSignature && activeAnimationRef.current) {
      return;
    }

    runtime.signature = chartSignature;
    runtime.phase = "animating";
    activeAnimationRef.current = true;

    drawConfidenceChart(svgElement, {
      workflowConfidence: targetScore,
    ...
    return () => {
      ...
      activeAnimationRef.current = false;
      if (
        runtimeRef.current.signature !== chartSignature &&
        runtimeRef.current.phase !== "done"
      ) {
