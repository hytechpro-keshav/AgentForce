...
/**
 * RC-8a — brute-force guard for the operator console login. Mirrors the
 * customer chat session limiter but reads the operator session rate-limit
 * config. In-memory sliding window keyed by client + route.
 */
@Injectable()
export class OperatorOrchestrationSessionRateLimitGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    const { rateLimitWindowMs, rateLimitMaxRequests } =
      this.config.operatorOrchestrationSession;
    ...
    if (existing.count >= rateLimitMaxRequests) {
      throw new HttpException(
        {
          error: "operator_orchestration_login_rate_limited",
