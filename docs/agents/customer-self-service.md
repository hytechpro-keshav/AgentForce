...
Move these capabilities to the external NestJS platform after Phase 0 passes evals:
...
Current deploy status: The Phase 0 action layer was deployed to the connected Salesforce org on 6 May 2026 with deploy job `0Afg5000007aZfHCAU`. A follow-up hardening deploy on 7 May 2026 with deploy job `0Afg5000007dTW0CAM` tightened escalation safety and passed 8/8 `CustomerSelfServiceActionsTest` tests.

Current temporary testing mode: Because the Developer Edition org hit the external email quota, the published planner bundle was temporarily switched into a manual-testing mode on 7 May 2026 with deploy job `0Afg5000007diegCAA`. In this temporary mode, the planner collects tester-provided account, contact, case, and email identifiers directly instead of routing through OTP verification first. Preview validation succeeded for direct account summary by email, service request creation, and escalation.

| AblyPro customer self-service capability       | What the use case expects                                                    | What is implemented now                                                                                                                                                                              | Coverage status                      | Next step                                                                                                               |
| ---------------------------------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------- |
| 24/7 intelligent support, zero wait time       | Customer can start support outside business hours across supported channels. | The Customer Self-Service planner bundle is active in Salesforce with working preview/runtime topics and configured customer surfaces. OTP mailbox delivery is the only current operational blocker. | Partial                              | Retest mailbox delivery after the GMT email-limit reset and confirm the pilot surface.                                  |
...
| Customer authentication / account lookup       | Customer is verified before account-specific data is shown.                  | Reuses the existing Service Customer Verification pattern in design; `Get_Customer_Account_Summary` requires verified Account, Contact, or email context.                                            | Partial                              | Confirm the production customer identity path: Experience Cloud session, Messaging identity, or OTP verification.       |
...
| Knowledge / RAG response generation            | Agent returns contextual answers grounded in knowledge.                      | Phase 0 allows approved Salesforce Knowledge only. External RAG is intentionally deferred.                                                                                                           | Pending for RAG                      | Add NestJS/LangChain/Pinecone in the external platform after native Agentforce workflow is proven.                      |
...
| Item                                   | Status           | Notes                                                                                                                                                                                                                                                                                                                                                                                                             |
| -------------------------------------- | ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OTP mailbox retest after GMT reset     | Pending external | The functional OTP flow works, but normal mailbox delivery must be retested after the external `SingleEmail` quota resets.                                                                                                                                                                                                                                                                                        |
...
## Temporary No-OTP Test Mode

The current published planner bundle is intentionally in a temporary manual-testing mode.

- OTP verification is temporarily bypassed.
- The planner now collects account id, contact id, case id, case number, or customer email directly from the tester when needed.
- This mode is only for manual Phase 0 testing while the org email quota is exhausted.

Validated preview behavior in this temporary mode:

- Direct account summary by customer email succeeded without OTP.
...
Re-enable verification-first behavior after manual testing is complete:
...
3. Validate the re-enabled OTP flow in Agent Preview after the external email quota resets.
