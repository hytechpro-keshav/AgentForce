...
> **Status:** **5-Pre + 5a + Railway E2E + 5b + 5c SHIPPED** (2026-06-18). Production flags `AI_API_ORCHESTRATOR_SCHEDULING_ENABLED=true` and `AI_API_ORCHESTRATOR_SCHEDULING_WRITES_ENABLED=true` on Railway `ai-api`. Smoke `ASSERT_SCHEDULING=1` and `ASSERT_SCHEDULING_WRITES=1` green. Live booking proof: workflow `wf-089804ec-45ee-449a-a5ab-59e7b3fd66db`, Case `500g500000axxLtAAI` (00001054), `SA-0086` — see §0.7. See §0.1, §0.5, §0.6, §0.7, and [`node5-field-service-prep-lessons.md`](../context/node5-field-service-prep-lessons.md).
> **Next:** 5d re-orchestration reconcile.
...
| **5c gated `ServiceAppointment` write**                | **Shipped (5c, 2026-06-18)** — live booking proof workflow `wf-089804ec-45ee-449a-a5ab-59e7b3fd66db`, `SA-0086` on Case `500g500000axxLtAAI` (00001054); `AI_API_ORCHESTRATOR_SCHEDULING_WRITES_ENABLED=true` on Railway | `salesforce-scheduling-write.gateway.ts`, `AgentforceSchedulingService.cls`, §0.7        |
...
### 0.7 Phase 5c state (2026-06-18) — gated `ServiceAppointment` write (**SHIPPED**)

Node 5 **books** the approved plan behind `AI_API_ORCHESTRATOR_SCHEDULING_WRITES_ENABLED`. Mirrors Node 4 Phase 4c: degrade-safe write gateway → Apex REST executor → post-approval `writeBack` only, with Salesforce owning DML + idempotency. Unblocked by Node 6 6a (`evaluateGuardrail` + `approvalDecision` in `writeBack`).
...
| Apex executor       | `AgentforceSchedulingService.cls` (+ `AgentforceSchedulingRest.cls`, `…ServiceTest.cls`) — `ServiceAppointment` + `AssignedResource`; initial `Status = None` (org FSL validation blocks insert in Scheduled/Dispatched)                                                                                                                                  |
| SF metadata         | `ServiceAppointment.Orchestrator_Workflow_Id__c` (idempotency) + `Agentforce_Scheduling_Node5` create/FLS                                                                                                                                                                                                                                                 |
| Config              | `AI_API_ORCHESTRATOR_SCHEDULING_WRITES_ENABLED=true` on Railway `ai-api` production                                                                                                                                                                                                                                                                       |
| Channel             | `scheduling.appointmentStatus = "booked"` + `appointmentReference` (`AppointmentNumber`, e.g. `SA-0086`) on success                                                                                                                                                                                                                                       |
| Smoke               | `scripts/smoke/all-3-nodes-deployed.sh` — `ASSERT_SCHEDULING_WRITES=1` (approvable Case, not escalated)                                                                                                                                                                                                                                                   |
| Tests               | ai-api **439** passed (gateway spec + graph 5c + service 5c); **10** Apex tests; typecheck + prettier clean                                                                                                                                                                                                                                               |
| **Live proof**      | **2026-06-18** — workflow `wf-089804ec-45ee-449a-a5ab-59e7b3fd66db` · Case `500g500000axxLtAAI` (00001054, University of Arizona / routine battery, local `SP-BATT-15X`) · guardrail `autoApprove` · status `done` · `scheduling.appointmentStatus=booked` · `appointmentReference=SA-0086` · `ServiceAppointment.Orchestrator_Workflow_Id__c` set in org |

**Rollout completed (2026-06-18):** deployed 5 metadata components to `AgentForce`, `Agentforce_Scheduling_Node5` already on OAuth run-as `chaudhary.keshav4u@gmail.com`, Railway flag flipped + `ai-api` redeployed (`e4abea2c`), booking smoke green. **Case selection:** do **not** use 00001050 (escalates); use a low-risk laptop Case (e.g. 00001054) that reaches `autoApprove` or `requireHumanApproval → approved`.
...
| **5c**    | Gated `ServiceAppointment` create after Node 6 approval (mirror Node 4 Phase 4c write-back)                                                                                                                                            | **Done** (2026-06-18) — live proof `wf-089804ec-45ee-449a-a5ab-59e7b3fd66db` / `SA-0086` (§0.7)                                             |
...
5. ~~**5c** gated `ServiceAppointment` writes (after Node 6)~~ **Done** (2026-06-18, §0.7).
