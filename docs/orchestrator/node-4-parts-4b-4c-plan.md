...
- `genAiFunctions/Create_Parts_Fulfillment/...` — `invocationTarget = AgentforcePartsFulfillmentService`, `isConfirmationRequired = true`.
...
- Perm set `Agentforce_Parts_Logistics_Node4.permissionset-meta.xml` — **add create/edit** object perms for `ProductRequest`, `ProductRequestLineItem`, and **add** `ProductTransfer` (read/create) + its fields (`SourceLocationId`, `DestinationLocationId`, `Product2Id`, `QuantityToTransfer`, `Status`) and `ProductRequest`/line `Orchestrator_Workflow_Id__c`.
